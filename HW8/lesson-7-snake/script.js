...
const game = {
    ...
    start() {
        ...
        timerId  = setInterval( () => this.move(), config.timerInterval);
        board.render();
        snake.render();
        food.render();
        score = 0;
    ...
    /**
     * Функция выполняет паузу игры. Повторное нажатие возобновляет и
     */
    pause() {
     ...
     */
    stop() {
        this.setGameStatus(GAME_STATUS_READY);
        clearInterval(timerId);
        this.gameOver();
    ...
    setGameStatus(status) {
        ...
        element.classList.remove(GAME_STATUS_READY, GAME_STATUS_STARTED, GAME_STATUS_PAUSED, GAME_STATUS_STOPPED);
